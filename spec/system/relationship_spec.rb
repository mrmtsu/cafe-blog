...
RSpec.describe "Relationships", type: :system do
  ...
  let!(:article2) { create(:article, :images, user: user2) }
  let!(:article3) { create(:article, :images, user: user3) }
  ...
  describe "フォロー中(following一覧)ページ" do
    ...
    context "ページレイアウト" do
      ...
      it "ユーザー情報が表示されていること" do
        ...
        expect(page).to have_content "投稿#{user.articles.count}件"
  ...
  describe "フィード" do
    ...
    it "フィードにフォロー中ユーザーの投稿が含まれていること" do
      expect(user.feed).to include article2
    end

    it "フィードにフォローしていないユーザーの投稿が含まれていないこと" do
      expect(user.feed).not_to include article3
    end
  end
end
